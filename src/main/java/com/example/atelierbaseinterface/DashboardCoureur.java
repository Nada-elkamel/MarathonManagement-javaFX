...
public class DashboardCoureur implements Initializable {
    ...
    public void updateRunner(ActionEvent actionEvent) {
        ...
        try {
            ...
            alert.setContentText("Impossible de mettre à jour le coureur");
            alert.showAndWait();
            e.printStackTrace();
        }
    }
    ...
    public void deleteRunner(ActionEvent actionEvent) {
        ...
        if (selectedRunner == null) {
            ...
            errorAlert.setContentText("Aucun coureur sélectionné.");
        ...
        confirmationAlert.setContentText("Voulez-vous vraiment supprimer ce runner ?");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                ...
                if (rowsDeleted > 0) {
                    ...
                    successAlert.setContentText("Le coureur a été supprimé avec succès.");
                    ...
                    Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                    errorAlert.setTitle("Erreur");
                    errorAlert.setContentText("Impossible de supprimer le coureur.");
                    errorAlert.showAndWait();
                }
            } catch (SQLException e) {
                Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                errorAlert.setTitle("Erreur");
                errorAlert.setContentText("Une erreur s'est produite lors de la suppression du coureur.");
                errorAlert.showAndWait();
                e.printStackTrace();
            }
        }

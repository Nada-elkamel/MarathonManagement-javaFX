    ...
    public void register(ActionEvent actionEvent) {
        ...
        try {
            ...
            if (ligneInsere > 0) {
                ...
                alert.setTitle("Confirmation de l'enregistrement");
                alert.setContentText("Enregistrement effectué dans la base");
                alert.showAndWait();
            ...
            alert.setContentText("Impossible d'enregistrer le temps du coureur");
            alert.showAndWait();
            e.printStackTrace();
        }
    }
    ...
    public void saveDataToFile(ActionEvent actionEvent) {
        ...
        if (file != null) {
            try {
                ...
                alert.setContentText("Les données ont été enregistrées dans le fichier.");
                alert.showAndWait();
            } catch (IOException e) {
                // Afficher une boîte de dialogue d'erreur en cas d'échec de l'enregistrement
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Erreur d'enregistrement");
                alert.setContentText("Une erreur s'est produite lors de l'enregistrement des données.");
                alert.showAndWait();
                e.printStackTrace();
            }
        }
